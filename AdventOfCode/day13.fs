...
let parse filename =
    let lines = filename |> ReadFile.readLines |> Array.toList
    let i = lines |> List.findIndex (fun l -> l = "")
...
let foldAt instruction set (x,y) =
    ...
    | at, fx when at = "x" ->
        if fx > x then Set.add (x,y) set
        else Set.add ( abs(x - fx * 2) , y) set
    | at, fy when at = "y" ->
        if fy > y then Set.add (x,y) set
        else Set.add ( x , abs(y - fy * 2)) set
    | _ -> failwith "wat"

let foldFirst (map, instructions: (string * int) list) =
    List.fold (foldAt instructions.[0]) Set.empty map
let part1 filename = filename |> parse |> foldFirst |> Set.count

let foldIns map instruction =
    map
    |> Set.fold (foldAt instruction) Set.empty

let fold (map: (int *int) list, instructions) =
    instructions
    |> List.fold foldIns (Set.ofList map)

let getMax fn set =
    set |> Set.toList |> List.map fn |> List.sort |> List.last
...
let draw set =
    ...
    [0..yMax]
    |> List.fold (fun lines y -> (List.fold (fun acc x -> $"{acc}{getChar set (x,y)}" ) "" [0..xMax])::lines) []
    |> List.rev
